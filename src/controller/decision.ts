...
function parsePatchBody(body: Request["body"]): UpdatableDecisionFields {
  if (!body)
    throw missingValue("req.body", new Error(`body is missing on request`));
...
app.patch("/decisions/:id", async (req, res, next) => {
  try {
    if (req.context.service !== Service.LABEL)
      throw forbiddenError(new Error());
...
app.put("/decisions", async (req, res, next) => {
  try {
    if (req.context.service !== Service.NORMALIZATION)
      throw forbiddenError(new Error());
