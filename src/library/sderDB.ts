...
if (process.env.MONGO_DB_URL == null)
  throw missingValue("process.env.MONGO_DB_URL", new Error());
...
async function _findAndUpdateDecision(
  ...
  if (!decisionWithId)
    throw unexpectedError(
      new Error("Upsert behave like there were no document and cannot create")
    );
