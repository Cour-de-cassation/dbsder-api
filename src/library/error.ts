...
export function notSupported(
  ...
  if (!error.message)
    error.message = `value: ${variableValue} is not supported to ${variableName}.`;
...
export function missingValue(variableName: string, error: Error): MissingValue {
  if (!error.message)
    error.message = `${variableName} is required but missing.`;
  return Object.assign(error, {
    variableName,
...
export function forbiddenError(error: Error): ForbiddenError {
  if (!error.message)
    error.message = `Resource needs to be logged to access. Currently forbidden.`;
  return Object.assign(error, {
