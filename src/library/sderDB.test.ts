...
const decisions =
    [
        { _id: 1 }, { _id: 2 }, { _id: 3 }, { _id: 4 }, { _id: 5 }, { _id: 6 }, { _id: 7 }
    ...
    describe('findDecisionsWithPagination', () => {
        it('should return without cursor if all results are there', async () => {
            const mocked = findDecisions
                ...
                .mockResolvedValueOnce({ decisions: [], length: decisions.length })
                .mockResolvedValueOnce({ decisions: [], length: decisions.length }) as unknown as typeof sderDb.findDecisions
        ...
        it('should return with cursors if there are', async () => {
            ...
            const mocked = findDecisions
                ...
                .mockResolvedValueOnce({ decisions: nextDecision, length: decisions.length }) as unknown as typeof sderDb.findDecisions
        ...
        it('should call findDecisions without pageFilters if no pages', async () => {
            const mocked = findDecisions
                .mockResolvedValue({ decisions: [], length: decisions.length }) as unknown as typeof sderDb.findDecisions
            const pages = {}
