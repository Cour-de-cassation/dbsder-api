...
export function parseModelWithId<T, K extends keyof T>(model: T, ...keys: K[]): IdParse<T, K> {
    try {
        return keys.reduce((acc, key) => {
            if (typeof acc[key] === "string") return {
                ...model,
                [key]: parseId(acc[key])
            }
            if (Array.isArray(acc[key]) && acc[key].every(_ => typeof _ === 'string')) return {
                ...model,
                [key]: parseId(acc[key])
            }
            throw new Error(`parseId: not parsable to ObjectId`)
        }, model as any) // Any because intermediate type during reduce.
    } catch(err) {
        throw new Error(`parseId: not parsable to ObjectId`)
    }
}

export function serializeModelWithId<T, K extends keyof T>(model: T, ...keys: K[]): Idserialize<T, K> {
    try {
        return keys.reduce((acc, key) => {
            if (acc[key] instanceof ObjectId) return {
                ...model,
                [key]: serializeId(acc[key])
            }
            if (Array.isArray(acc[key]) && acc[key].every(_ => _ instanceof ObjectId)) return {
                ...model,
                [key]: serializeId(acc[key])
            }
            throw new Error(`parseId: not parsable to ObjectId`)
